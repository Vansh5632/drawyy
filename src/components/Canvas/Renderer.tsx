...
const Renderer: React.FC<RendererProps> = ({
  ...
  useEffect(() => {
    ...
    mathResults,
    refToUse,
  ]); // Added refToUse to dependency array
  ...
  const drawMathResults = (
    ctx: CanvasRenderingContext2D,
    shapesToRender: Shape[]
  ) => {
    ...
    switch (shape.type) {
      ...
      case "line":
        if (shape.points && shape.points.length >= 2) {
          ...
          // Fallback to startPoint and endPoint if points array is incomplete
          drawLine(ctx, shape.startPoint, shape.endPoint);
        } else {
          console.warn("Invalid line shape", shape);
        }
        break;
      case "rectangle":
